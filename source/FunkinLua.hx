...
class FunkinLua {
	...
	public function new(script:String) {
		...
		Lua_helper.add_callback(lua, "makeLuaVideoSprite", function(tag:String, image:String, x:Float, y:Float) {
			tag = tag.replace('.', '');
			resetSpriteTag(tag);
			var leSprite:ModchartSprite = new ModchartSprite(x, y);
			if(image != null && image.length > 0)
		...
		Lua_helper.add_callback(lua, "startVideo", function(videoFile:String) {
			...
			return true;
				...
				Lua_helper.add_callback(lua, "startLoopVideo", function(videoFile:String) {
			#if VIDEOS_ALLOWED
			if(FileSystem.exists(Paths.video(videoFile))) {
				PlayState.instance.StartLoopVideo(videoFile);
				return true;
			} else {
				luaTrace('StartLoopVideo: Video file not found: ' + videoFile, false, false, FlxColor.RED);
			}
			return true;
